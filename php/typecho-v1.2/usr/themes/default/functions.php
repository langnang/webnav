...
EOD;
    ...
    // 主题关联根标识
    $webstack_root_meta = new Typecho_Widget_Helper_Form_Element_Text('webstack_root_meta', NULL, '1', _t('主题关联根标识'), _t('主题关联根标识，如 0，留空则默认为 0(mid=0)，即加载全部文章数据'));
    $form->addInput($webstack_root_meta);

    // 标签导航功能
    $zmki_navs = new Typecho_Widget_Helper_Form_Element_Radio('zmki_navs', array('0' => _t('禁用'), '1' => _t('启用')), '0', _t('标签导航功能'), _t("是否在主页上启用标签导航"));
    $form->addInput($zmki_navs);
    // 是否启用网址详情页
    $zmki_content_detail = new Typecho_Widget_Helper_Form_Element_Radio('zmki_content_detail', array('0' => _t('禁用'), '1' => _t('启用')), '0', _t('是否启用网址详情页'), _t("点击导航需先跳转至详情页"));
    $form->addInput($zmki_content_detail);
    // PC端每行显示数量
    ...
    // 顶部模块
    $zmki_top_main = new Typecho_Widget_Helper_Form_Element_Radio('zmki_top_main', array('0' => _t('禁用'), '1' => _t('启用')), '0', _t('<svg class="icon zmki_aliico" aria-hidden="true"><use xlink:href="#icon-prompt"></use></svg> <span style="color: #608cee; margin-right:0px;">顶部</span><span style="color: #fb5962;margin-right:0px;">多色</span><span style="color: #fbb359;margin-right:0px;">模块</span><span style="color: #53bf6b;margin-right:0px;">开关</span>'), _t("是否开启网站顶部四项多色小模块"));
